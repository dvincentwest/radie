...
from dataquick import structures
from . import exceptions
from .structures.dataquickframe import DataQuickFrame

loaders = dict()


class Loader(object):
    """object used to map file-extensions to functions that load DataQuickFrame subclasses

    The loader object is a place holder that connects a list of file-extensions to a a function that will read those
    files and DataQuickFrame subclass that the function will return.  Its main purpose is for book-keeping to allow
    ...
    instances) of the `DataQuickFrame` subclass specified in the `cls` attribute

    Attributes
    ----------

    cls: typing.Type(DataQuickFrame)
        the `DataQuickFrame` sub-class that the loader function will load
    extensions : list, str
    ...
    def __init__(self, loader_function, cls, extensions, label):
        ...
        loader_function : function
            the function to load the DataFrame from a file, expects filename argument
        cls: typing.Type(DataQuickFrame)
            the `DataQuickFrame` sub-class that the loader function will load
        extensions : list, str
            a string or list of strings specifying the file types
    ...
    def load(self, filename):
        """load a `DataQuickFrame` object from the loader function and return
...
def load_csv(fname, encoding=None):
    ...
    dqf = DataQuickFrame(df.dropna(axis=("index", "columns"), how="all"), name=name)
    return dqf


def load_dqftxt(fname):
    """load files from the `DataQuickFrame.savetxt` method
    ...
    -------
    DataQuickFrame
        fully specified datastructure from the dqftxt file
    ...
    with open(fname, "r") as fid:
        ...
        try:
            ...
            raise exceptions.IncorrectFileType("dqf file missing required class for file-type information")
        else:
            try:
                cls = structures.structures[cls_key]  # type: typing.Type[DataQuickFrame]
            except KeyError:
                raise exceptions.LoaderException("dqf class {:s} is not supported by any known DQF Structure "
        ...
        if not cls.required_columns().issubset(df.columns):
            raise exceptions.LoaderException("DataFrame Columns do not match the required columns for the specified "
                                             "DataQuickFrame Structure")

        dqf = cls(df.dropna(axis=("index", "columns"), how="all"), **meta_data)
        return dqf


dqf_loader = Loader(load_dqftxt, DataQuickFrame, (".dqf"), "DataQuickFrame text file")
register_loaders(dqf_loader)


def load_file(fname):
    """the catch-all convenience function to automatically load data-files

    This function is at the heart of one of the goals of DataQuick.  The idea is that any datafile that is supported
    by the dataquick structures and loaders can be passed to this function without needing to specify anything, and the
    ...
    on to the next loader function.  The first time a valid DataQuickFrame is returned, the function breaks the look and
    ...
    Returns
    -------
    DataQuickFrame or list or tuple
        the output of the automatically determined loader function, could be a
        single DataQuickFrame or a list of DataQuickFrames
    ...
    for loader in loaders_:  # type: Loader
        try:
            ...
            if isinstance(dfs, DataQuickFrame):
                print("successfully loaded file as {:}".format(type(dfs).__name__))
                return dfs
            elif type(dfs) in (list, tuple):
                df_list = [df for df in dfs if isinstance(df, DataQuickFrame)]
                if not df_list:
                    raise exceptions.LoaderException("function {:s} did not return any DataQuickFrame "
