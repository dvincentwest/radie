...
from dataquick import exceptions
from dataquick.loaders import Loader, register_loaders
from dataquick.plugins.structures.powderdiffraction import PowderDiffraction
...
def load_raw(fname, name=None):
    ...
    If multiple ranges exist in the RAW file a list of PowderDiffraction DataFrame
    ...
    df_xrd : single PowderDiffraction or list of them
        PowderDiffraction DataFrame based on XRD data
    ...
    if isinstance(dfs, PowderDiffraction) or type(dfs) in (list, tuple):
        ...
        raise TypeError("Unrecognized DataFrame type: {:}".format(type(dfs)))
