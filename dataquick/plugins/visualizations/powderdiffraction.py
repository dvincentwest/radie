...
from dataquick.qt import colors, uchars
from dataquick.qt import plotlist
from dataquick.qt.visualizations import base, register_visualizations
from dataquick.plugins.structures import powderdiffraction
from dataquick.plugins.visualizations.ui.powderdiffraction import Ui_PowderDiffraction
...
class VisPowderDiffraction(base.Visualization, Ui_PowderDiffraction):
    ...
    def __init__(self, parent=None):
        ...
        self.listView_datasets.setItemClass(DQFItem)
    ...
    def recalculateXValues(self):
        for item in self.listView_datasets.iterItems():  # type: DQFItem
            if item.checkState:
                item.plotDataItem.setData(
                    x=self.xValues(item),
    ...
    def addCurves(self, listItems):
        ...
        for item in listItems:  # type: DQFItem
            item.plotDataItem.setData(
                x=self.xValues(item),
                y=self.yValues(item),
                pen=self.nextColor
