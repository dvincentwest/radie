...
class DQFItem(object):
    def __init__(self, ref, item_list, name=None):
        """
        Parameters
        ----------
        ref : DQFReference
        item_list : DQFItemList
        name : str
        """
        self.ref = ref
        self.list = item_list
...
class DQFItemList(list):
    def __init__(self, iterable=None):
        super(DQFItemList, self).__init__()
        if not iterable:
            return

        for item in iterable:
            if not isinstance(object, DQFItem):
                raise TypeError("Only DQFReference objects are allowed")
            self.append(item)
    ...
    def itemClass(self, cls):
        if not issubclass(cls, DQFItem):
            raise TypeError("cls must be a DQFItem subclass not {:}".format(cls.__name__))
        self._itemClass = cls
    ...
    def append(self, ref):
        ...
        ref : DQFReference
        """
        if not isinstance(ref, DQFReference):
            raise TypeError("Only DQFReference objects are allowed, not type: {:}".format(type(DQFReference)))
        item = self.itemClass(ref, self)
        super(DQFItemList, self).append(item)

    def __setitem__(self, key, value):
        if not isinstance(value, DQFReference):
            raise TypeError("Only DQFReference objects are allowed")
        super(DQFItemList, self).__setitem__(key, value)


class DQFListModel(QtCore.QAbstractItemModel):
    """
    ...
    def __init__(self, dflist: DQFItemList):
        super(DQFListModel, self).__init__()
        if not isinstance(dflist, DQFItemList):
            raise TypeError("DQFListModel only supports DQFItemLists")
        self.dflist = dflist  # type: DQFItemList
    ...
    def data(self, index: QtCore.QModelIndex, role: int = ...):
        ...
        if col == 0:
            if role == QtCore.Qt.CheckStateRole:
                item = self.dflist[index.row()]  # type: DQFItem
...
class DQFListView(QtWidgets.QTreeView):
    modelClass = DQFListModel

    def __init__(self, parent=None):
        super(DQFListView, self).__init__(parent)
        self.setSelectionMode(self.ExtendedSelection)
        self.setDragDropMode(self.DragDrop)
        self.setAcceptDrops(True)
        self.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.customContextMenuRequested.connect(self.rightClicked)
        self.supportedClasses = (DataQuickFrame,)  # support all DataFrame classes by default
    ...
    def model(self) -> DQFListModel:
        return super(DQFListView, self).model()
    ...
    def setModel(self, model: DQFListModel):
        if not isinstance(model, DQFListModel):
            raise TypeError("This View only supports models of type {:}".format(DQFListModel.__name__))
        super(DQFListView, self).setModel(model)

    def isSupported(self, ref: DQFReference):
        supported = isinstance(ref.df, self.supportedClasses)
        return supported

    def addDataFrames(self, *refs: DQFReference):
        for ref in refs:
    ...
    def iterItems(self):
        ...
        """
        for item in self.model().dflist:  # type DQFItem
            yield item
    ...
    def rightClicked(self, pos: QtCore.QPoint):
        ...
        if num_dataframes > 0:
            if num_dataframes == 1:
                deleteItem = QtWidgets.QAction("Delete DataFrame")
            else:
    ...
    def dragEnterEvent(self, event):
        if event.mimeData().hasUrls:
            ...
            super(DQFListView, self).dragEnterEvent(event)
        ...
        if source is self:  # internal move operation
            ...
            if isinstance(source, QtWidgets.QTreeView):
                ...
                for index in source.selectionModel().selectedRows(column=0):  # type: QtCore.QModelIndex
                    pointer = index.internalPointer()  # type: DQFReference
                    if self.isSupported(pointer):
                        valid_drops.append(pointer)
                    elif isinstance(pointer, DQFReference):
...
class DQFAccessorDelegate(QtWidgets.QItemDelegate):
    ...
    def setModelData(self, editor: QtWidgets.QComboBox, model: DQFListModel, index: QtCore.QModelIndex):
...
class DQFSeriesListView(DQFListView):
    def __init__(self, parent=None):
        super(DQFSeriesListView, self).__init__(parent)
...
def test(tree=False):
    """
    Instantiate a DQFTreeView, add in some data and test the drag and drop
    features.
    """
    from ..plugins.examples import example_powderdiffraction, example_vsm
    from .masterdqftree import DQFTreeView
    import sys
    ...
    if not tree:
        ...
        lv = DQFXYListView()
    lv.setItemClass(DQFItem)  # cannot add items to the list before calling this
    hlay.addWidget(tv)
    hlay.addWidget(lv)
