...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("DataQuick")
        self.setWindowIcon(QtGui.QIcon(cfg.dataquick_icon))
        ...
        self.treeView_dataFrames = masterdqftree.DQFTreeView()
    ...
    def dropEvent(self, a0: QtGui.QDropEvent):
        ...
        for url in urls:  # type: QtCore.QUrl
            ...
            if isinstance(dfs, DataQuickFrame):
                dfs = [dfs]
            for df in dfs:  # type: DataQuickFrame
                try:
                    self.treeView_dataFrames.addDataFrame(df)
                except Exception as e:
                    err_msg += "\nError: {:}.\nSomething wrong could not load {:} as a DataQuickFrame Object".format(str(e), df)
                    continue
    ...
    def importDataFrameFromClipboard(self):
        df = DataQuickFrame.from_clipboard()
        df.metadata["name"] = "DF - clipboard"
        self.treeView_dataFrames.addDataFrame(df)


def run(debug=False):
    fn.set_process_id("dataquick")
    # fn.setup_style()
